layui.config({
	...
	//创建一个编辑器
 	...
 	form.on("submit(addNews)",function(data){
 		...
 		addNews.newsKey = $(".newsKey").val();
 		addNews.newsAbstract = $(".newsAbstract").val();
 		addNews.newsContent = $(".newsContent").val();
 		// addNewsArray.unshift(JSON.parse(addNews));
 		// window.sessionStorage.setItem("addNews",JSON.stringify(addNewsArray));
 		//弹出loading
 		var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.post("/crm/index.php/Home/News/addNews", addNews,
	      	function(data){
	      		if($.parseJSON(data).code == 0){
	      			top.layer.close(index);
					top.layer.msg("销售活动添加成功！");
		 			layer.closeAll("iframe");
			 		//刷新父页面
			 		parent.location.reload();
	      		}else{
	      			layer.alert("销售活动保存失败！")
	      		}          		
		});
 		return false;
 	})
